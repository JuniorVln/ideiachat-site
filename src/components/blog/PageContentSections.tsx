...
export async function PageContentSections({
  ...
  /** CTA inline no meio do corpo — após a secção central (sem travar a leitura). */
  const midCtaIdx = rest.length >= 3 ? Math.floor(rest.length / 2) : -1;
          ...
          <div key={idx}>
            ...
            {idx === midCtaIdx && (
              <BlogInlineCta
                ...
                keyword={focusKeyword ?? pageTitulo}
                origem="blog-inline-meio"
                eyebrow="Atendimento sem dor de cabeça"
                title="Está com o WhatsApp instável ou sob risco de banimento?"
                description="A gente migra o seu atendimento para a API Oficial da Meta e organiza tudo em um só lugar. Deixe seu contato e mostramos como."
                ctaLabel="Quero proteger meu número"
              />
            )}
          </div>
        );
      ...
      {visible.length > 0 && (
        <>
          ...
          <BlogInlineCta
            ...
            whatsappNumber={whatsappNumber}
            keyword={focusKeyword ?? pageTitulo}
            origem="blog-inline-fim"
            eyebrow="Pronto para começar?"
            title="Centralize e profissionalize o seu atendimento hoje"
            description="Fale com um especialista, tire suas dúvidas e veja os planos na prática — sem compromisso."
