...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) return { title: "Solução não encontrada" };

  const { pagina, visuals } = data;
  const canonical = `${SITE_URL}${LP_BASE_PATH}/${pagina.slug}`;
...
export default async function SolucaoSlugPage({ params, searchParams }: Props) {
  const { slug: rawSlug } = await params;
  const data = await loadBlogPost(rawSlug);
  if (!data) notFound();
  ...
  return (
    <>
      ...
      <main aria-label="Conteúdo comercial da solução">
        {focusKeyword ? (
          <p className="sr-only">
